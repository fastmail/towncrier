...
# And services
curl -u $AUTH -i http://localhost:3000/admin/api/v1/services -F id=web-client -F order=10 -F name='Web client and mobile app' -F description='Access to services via the web client and mobile app'
...
curl -u $AUTH -i http://localhost:3000/admin/api/v1/services -F id=contacts -F order=70 -F name='Contacts (CardDAV)' -F description='Contacts access via the CardDAV protocol'
curl -u $AUTH -i http://localhost:3000/admin/api/v1/services -F id=support -F order=80 -F name='Support System' -F description='Fastmail Support System'
